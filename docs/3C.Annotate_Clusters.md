...
features <- rownames(renin_seurat)
features <- features[!grepl("^Rp[sl][[:digit:]]|^Rplp[[:digit:]]|^Rpsa", features)]
features <- features[!grepl("^mt-", features)]
features <- features[!grepl("^Hb[a-b]", features)]
features <- features[!grepl("^[0-9]", features)]

p_volcano <- plotVolcano(
    renin_seurat,
    group.1         = "JG",
    group.2         = c("tSMC_1", "tSMC_2", "tSMC_3", "fbJG"),
    test            = "MAST",
    gene_list       = features,
    no_shape        = TRUE,
    outdir          = "figures/DEGs",
    highlight_genes = c("Ren1", "Akr1b7", "Acta2", "Col4a2", "Cdh13")
)

if (!is.null(p_volcano)) {
    ggsave(paste0("figures/", sample_name, "_JG-vs-Transformed_volcano_", Sys.Date(), ".svg"),
           plot = p_volcano, width = 180, height = 180, units = "mm", dpi = 300)
}
```
```